...
| <a name="input_custom_s3_buckets"></a> [custom\_s3\_buckets](#input\_custom\_s3\_buckets) | S3 buckets that should have shared access from AWS Batch | `list(string)` | n/a | yes |
| <a name="input_db_engine_version"></a> [db\_engine\_version](#input\_db\_engine\_version) | n/a | `string` | `"16"` | no |
...
| <a name="input_enable_deletion_protection"></a> [enable\_deletion\_protection](#input\_enable\_deletion\_protection) | Enable deletion protection for RDS database | `bool` | `true` | no |
| <a name="input_enable_iam_authentication"></a> [enable\_iam\_authentication](#input\_enable\_iam\_authentication) | Enable IAM database authentication | `bool` | `true` | no |
...
| <a name="input_extra_metadata_service_env_vars"></a> [extra\_metadata\_service\_env\_vars](#input\_extra\_metadata\_service\_env\_vars) | Additional environment variables for the metadata service container | `map(string)` | `{}` | no |
...
| <a name="input_job_state_time_limit_actions"></a> [job\_state\_time\_limit\_actions](#input\_job\_state\_time\_limit\_actions) | One or more Batch job queue job state time limit rules. Each entry cancels (action CANCEL) jobs stuck in RUNNABLE after max\_time\_seconds for the given AWS Batch reason code. See https://docs.aws.amazon.com/batch/latest/userguide/job_stuck_in_runnable.html | <pre>list(object({<br>    max_time_seconds = number<br>    reason           = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_step_functions_dynamodb_point_in_time_recovery_enabled"></a> [step\_functions\_dynamodb\_point\_in\_time\_recovery\_enabled](#input\_step\_functions\_dynamodb\_point\_in\_time\_recovery\_enabled) | Enable Point-in-Time Recovery for the Step Functions state DynamoDB table | `bool` | `false` | no |
...
| <a name="input_ui_alb_drop_invalid_header_fields"></a> [ui\_alb\_drop\_invalid\_header\_fields](#input\_ui\_alb\_drop\_invalid\_header\_fields) | Indicates whether invalid header fields are dropped by the UI Application Load Balancer | `bool` | `false` | no |
...
| <a name="input_ui_alb_ssl_policy"></a> [ui\_alb\_ssl\_policy](#input\_ui\_alb\_ssl\_policy) | Name of the SSL policy for the UI HTTPS listener (e.g. ELBSecurityPolicy-TLS13-1-2-2021-06). If null, the load balancer default is used. | `string` | `null` | no |
...
| <a name="input_ui_allow_security_groups"></a> [ui\_allow\_security\_groups](#input\_ui\_allow\_security\_groups) | A list of security groups the UI will be available to | `list(string)` | `[]` | no |
